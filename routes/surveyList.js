...
router.route("/").get(async (req, res) => {
    ...
    router.route("/delete/:id").post(async (req, res) => {
      try {
        ...
        await surveyData.removeSurvey(id);
        const surveyCollection = await surveyData.getSurveyList(id);
        console.log(surveyCollection);
        
        res.status(200).render("surveyList", { title: "Survey List", surveyCollection});
      } catch (e) {
